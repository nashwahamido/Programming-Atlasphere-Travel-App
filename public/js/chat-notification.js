(function () {
  ...
  function updateBadge() {
    ...
    if (groupsBadge) {
      ...
      groupsBadge.style.display = count > 0 ? 'flex' : 'none';
    }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (chatTarget) {
      var m = window.location.pathname.match(/\/groups\/([^\/]+)$/);
      if (m) clearGroup(m[1]);
    }
  });
  ...
  function initSocket(groups) {
    ...
    socket.on('new-message', function (msg) {
      ...
      // Ignore if user is already viewing this group page
      var m = window.location.pathname.match(/\/groups\/([^\/]+)$/);
      if (m && String(m[1]) === String(msg.groupId)) return;
